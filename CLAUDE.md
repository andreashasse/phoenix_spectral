...
3. For each route, extract the 5-arity function spec from the controller's type info
...
- Call handlers as 5-arity `action(conn, path_args, query_params, headers, body)` instead of Phoenix's `action(conn, params)`
...
- Controller actions use **5-arity** `(conn, path_args, query_params, headers, body)` and return `{status_code, headers, body}`
